...
def matplotlib_line_example2():
    ...
    """
    fig, ax = matplotlib_line_example(
        np.array([1, 2, 3, 4, 5]), np.array([5, 3, 5, 3, 5]))
    ax.set(xlabel='some linear list',
           ylabel='another list changing values', title='Another example')
    ax.grid()
    plt.show()
...
def matplotlib_bar_chart(x: np.array) -> Tuple:
    ...
    # I was not getting Label Attribute. So, I thought of creating Indexes as Labels
    data_len = len(x)
    unique_bars = np.arange(data_len)

    # Plotting the required bar chart
    fig, ax = plt.subplots()
    ax.bar(unique_bars, x)
    return fig, ax
...
def matplotlib_pie_chart(x: np.array) -> Tuple:
    ...
    # Plotting the required pie chart
    fig, ax = plt.subplots()
    ax.pie(x)
    return fig, ax
...
def matplotlib_polar_chart(x: np.array, y: np.array) -> Tuple:
    ...
    # Plotting the required polar chart
    fig = plt.figure()
    ax = fig.add_subplot(polar=True) # Setting up the Polar Flag for generating Polar Plot
    ax.plot(x, y)
    return fig, ax
...
def matplotlib_heatmap_chart(matrix: np.array) -> Tuple:
    ...
    # Plotting the required heatmap chart
    fig, ax = plt.subplots()
...
def matplotlib_composite_line_bar(x: np.array) -> Tuple:
    ...
    # # I was not getting Label Attribute. So, I thought of creating Indexes as Labels for Bar Graph
    data_len = len(x)
    unique_bars = np.arange(data_len)
...
def matplotlib_subgraphs(fig1, fig2, fig3, fig4) -> Tuple:
    ...
    # Not Implemented as we were told that the parameters are inappropriate and thus
    # we are not supposed to implement this one.
    return None
...
def plotly_bar_chart(df: pd.DataFrame):
    ...
    # Plotting the required plotly bar chart
    fig = px.bar(df, x="x", y="y", title='My Bar Graph')
    return fig
...
def plotly_pie_chart(df: pd.DataFrame):
    ...
    # Plotting the required plotly pie chart
...
def plotly_histogram(df: pd.DataFrame, n_bins: int):
    ...
    # Plotting the required plotly histogram
...
def plotly_heatmap_chart(df: pd.DataFrame):
    ...
    # Plotting the required plotly heatmap chart
...
def plotly_table(df: pd.DataFrame):
    ...
    # Plotting the required plotly table
    trace = go.Table(
        header=dict(values=list(df.columns)),
        cells=dict(values=[df['grade of class 0'], df['grade of class 1'], df['grade of class 2'], df['grade of class 3'], df['grade of class 4'], df['grade of class 5'], df['grade of class 6'], df['grade of class 7'], df['grade of class 8'], df['grade of class 9']]))
    data = [trace]
...
def plotly_composite_line_bar(df: pd.DataFrame):
    ...
    # Plotting the required composite chart
    fig = go.Figure()
...
def plotly_subgraphs(df: pd.DataFrame):
    ...
    # Creating an empty figure with 4 internal subplots
    ...
    # Populating the third subplot
    fig.add_trace(
        go.Bar(x=df['x3'], y=df['y3']),
        row=2, col=1
    )

    # Populating the fourth subplot
    fig.add_trace(
        go.Scatter(x=df['x4'], y=df['y4'], fill='tozeroy'),
        row=2, col=2
    )
...
if __name__ == "__main__":
    ...
    # We were told that we are not supposed to implement the below method. So, I have commented it.
    #matplotlib_subgraphs(fig1, fig2, fig3, fig4)

    df = pd.DataFrame(dict(x=x, y=y, z=x + y))
    df_matrix = pd.DataFrame(matrix_2D, columns=[f'grade of class {x}' for x in range(
        10)], index=[f'professor {x}' for x in range(10)])
    plotly_bar_chart(df)
    plotly_pie_chart(df)
